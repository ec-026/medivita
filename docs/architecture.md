...
sequenceDiagram
  ...
  par one search per enabled source
    R->>S: site:trusted-domain query
  end
  S-->>R: validated HTTPS results
  par top page per source
    R->>P: bounded fetch with redirect checks
  end
  P-->>R: extracted text or snippet fallback
  R->>L: structured decision + E1…En evidence
  alt sufficient
    L-->>R: final answer + used evidence IDs
  else search_more
    L-->>R: up to 4 enabled-source targeted searches
    R->>S: one bounded follow-up retrieval round
    R->>L: required final structured answer
  end
...
The controller performs exactly one generation call for a sufficient first answer and at most two calls for `search_more`. A second `search_more`, missing final answer, or unusable structured result becomes `AI_INVALID_RESPONSE`; there is never a third call.
...
Trace fields are allowlisted and bounded. They may include stage/status, tool, source, safe domain-restricted query, actual contributing DDGS backend, counts, retrieval type, provider/model, research round, and elapsed time. They never contain authorization data, prompts, histories, raw page/evidence content, system messages, model output internals, hidden reasoning, or chain-of-thought. Demo mode reports only that a deterministic response ran with no external tools.
...
Evidence is request-local and receives backend-owned IDs (`E1`, `E2`, …). It carries the source ID/name/domain, title, validated URL, snippet, compact content, originating query, and retrieval type. Follow-up evidence is canonical-URL deduplicated and appended within the global character cap. The model sees evidence IDs but not authority to create citations. The backend maps valid model-selected IDs exactly. If none are valid, it deterministically maps up to three ranked items from the evidence actually supplied to that model call, after rechecking the enabled source, trusted HTTPS domain, and canonical URL. A connected answer is never returned with an empty source list.

## LangChain and LLM providers

Primary connected mode composes `ChatGroq` with a `ChatPromptTemplate`, bounded message history, Pydantic JSON-schema output, low reasoning effort, hidden reasoning, and low retries. The default model is `openai/gpt-oss-20b`. Chat and Health Check use distinct answer schemas through the same bounded controller.
