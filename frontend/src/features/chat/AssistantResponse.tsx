...
export function AssistantResponse({ response }: { response: ChatResponse; latest?: boolean }) {
  const responseLabel = response.mode === 'demo' ? 'Demo response' : response.response_kind === 'direct' ? 'Direct response' : response.response_kind === 'clarification' ? 'Clarifying question' : 'Researched response'
  return <article className="pb-8">
    <header className="mb-3.5 flex items-center gap-3"><BrandMark className="h-8 w-8 shrink-0" /><div className="leading-none"><p className="text-[15px] font-semibold leading-5 text-ink">MediVita</p><p className="mt-0.5 text-[13px] leading-4 text-faint">{responseLabel}</p></div></header>
