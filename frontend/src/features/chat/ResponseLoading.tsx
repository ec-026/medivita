...
export function ResponseLoading({ trace }: { trace?: ResearchTraceEvent[] }) {
  return <div className="pb-9" aria-label="MediVita is researching a response" role="status">
    ...
    {trace?.length ? <ResearchActivity events={trace} running /> : <div className="flex items-center gap-2 py-2 text-[12px] text-muted"><span className="h-2 w-2 animate-pulse rounded-full bg-mint-dark motion-reduce:animate-none" />Starting trusted research…</div>}
  </div>
}
